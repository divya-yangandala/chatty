...
export class ImageUtils {
  ...
  static checkFileSize(file) {
    ...
    if (!isValid) {
      fileError = `File ${file.name} is not accepted`;
    }
  ...
  static checkFile(file) {
    if (!ImageUtils.validateFile(file)) {
      return window.alert(`File ${file.name} is not accepted`);
    }
  ...
  //function used to convert imageURL to base64 encoded string
  ...
  static convertRGBToHex(red, green, blue) {
    ...
    // suppose our RGB is 182, 142, 154 the it will convert it to #457ccd
