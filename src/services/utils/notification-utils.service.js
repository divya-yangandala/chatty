...
import { cloneDeep, findIndex, find, remove, sumBy } from 'lodash';
import { timeAgo } from '@services/utils/timeago.utils';

export class NotificationUtils {
  // notification utils function that will handle the insert notification event,update notification and delete notification.
    ...
    socketService?.socket?.on('insert notification', (data, userToData) => {
      if (profile?._id === userToData.userTo) {
        ...
        if (type === 'notificationPage') {
          ...
          setNotificationsCount(mappedNotifications);
        }
      }
    });
  ...
  // map notification data into dropdown list
  ...
  static async markNotificationsAsRead (notificationId, notification, setNotificationDialogContent) {
