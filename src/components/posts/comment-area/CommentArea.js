...
import { avatarColors, reactionsMap } from '@services/utils/static.data';
...
const CommentArea = ({ post }) => {
  ...
  const addReactionPost = async (reaction) => {
    try {
      ...
      if (!Object.keys(reactionResponse.data.reactions).length) {   // IF User is adding reaction for the first time
    ...
    <div className="comment-area" data-testid="comment-area">
      <div className="like-icon reactions">
        ...
        <div className="reactions-container app-reactions">
          <Reaction handleClick={addReactionPost} />
        </div>
      </div>
