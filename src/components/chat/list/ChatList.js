...
export const ChatList = () => {
  ...
  const addSelectedUserToList = useCallback((user) => {
    ...
    const findUser = find(chatMessageList, (chat) =>
    ...
    if (!findUser) {
      ...
      if (!chatList.length) {
        ...
        const userTwoName = newUser?.receiverUsername !== profile?.username ?
          newUser?.receiverUsername : newUser?.senderUsername;
  ...
  // this is when a user already exist in the chat list
  const addUsernameToUrlQuery = async(user) => {
    try {
      ...
      const userTwoName = user?.receiverUsername !== profile?.username ?
        user?.receiverUsername : user?.senderUsername;
      const receiverId =  user?.receiverUsername !== profile?.username ?
  ...
  useEffect(() => {
    if (selectedUser && componentType === 'searchList') {
      addSelectedUserToList(selectedUser);  // If it comes from searchList component
    ...
    <div className="conversation-container">
        ...
        <div className="conversation-container-search" data-testid="search-container">
            ...
            <Input id="message" name="message" type="text" value={search} className="search-input" labelText="" placeholder="Search"
              handleChange={(event) => { setIsSearching(true); setSearch(event.target.value)}}
            />
            ...
            <div className="conversation">
                ...
                >
                    <div className="avatar">
                        <Avatar
                          name={data.receiverUsername === profile?.username ?
                            profile?.username : data?.senderUsername}
                          bgColor={data.receiverUsername === profile?.receiverAvatarColor ?
                            data?.receiverAvatarColor : data?.senderAvatarColor}
                          textColor="#ffffff" size={40}
                          avatarSrc={data.receiverUsername !== profile?.username ?
                            data?.receiverProfilePicture : data?.senderProfilePicture
                          } />
                    </div>
                    ...
                    {data?.createdAt && <div className="created-date">
