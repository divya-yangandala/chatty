...
const Sidebar = () => {
  ...
  const navigateToPage = (name, url) => {
    ...
    // Even after leaving the page the msg listening event is called. just using off() to stop listening so that there's no memory leak
  ...
  const createChatUrlParams = useCallback((url) => {
    if (chatList.length) {
      ...
      return `${url}?${createChatUrlParams(params)}`
    }
    return url;
  }, [chatList, profile]);

  const markMessagesAsRead = useCallback(async (user) => {
    try {
  ...
  // useEffect(() => {
  //   console.log("chatPageName:   ", chatPageName);
  ...
  useEffect(() => {
    if (chatPageName === 'Chat') {
      if (chatList.length) {
        const params = ChatUtils.chatUrlParams(chatList[0], profile);
        const url = `/app/social/chat/messages?${createSearchParams(params)}`;

        // ⭐️ Only join room once
        if (!ChatUtils.hasJoinedRoom) {
          ChatUtils.joinRoomEvent(chatList[0], profile);
          ChatUtils.hasJoinedRoom = true;
        }

        navigate(url, { replace: true });
        if (!chatList[0].isRead) {
          markMessagesAsRead(chatList[0]);
        }
      }
    }
  }, [chatPageName, chatList]);
