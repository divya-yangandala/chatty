...
const Header = () => {
  ...
  const [isSettingsActive, setIsSettingsActive] = useDetectOutsideClick(settingRef, false);
  ...
  const onMarkAsRead = async (notification) => {
    try {
      NotificationUtils.markNotificationsAsRead(notification?._id, notification, setNotificationDialogContent);
    } catch (error) {
      Utils.dispatchNotification(error.response.data.message, 'error', dispatch);
    }
  }
    ...
    try {
      ...
      const userTwoName =
        notification?.receiverUsername !== profile?.username
          ? notification?.receiverUsername : notification?.senderUsername;
  ...
  const onLogout = async () => {
    try {
      setLoggedIn(false);
      Utils.dispatchNotification('Looged Out Successfully', 'success', dispatch);
      Utils.clearStore({ dispatch, deleteStorageUsername, setLoggedIn, deleteSessionPageReload });
  ...
  useEffect(() => {
    ...
    setEnvironment(env);
    // we get the sum of chat from chatList where isRead property is false
  ...
  }, [profile, notifications, messageNotifications, location, dispatch]);
            ...
            <div className="header-navbar">
              ...
              <ul className="header-nav">
                <li className="header-nav-item active-item"
                  onClick={() => { setIsMessageActive(false); setIsNotificationActive(true); setIsSettingsActive(false) }}>
                ...
                <li className="header-nav-item active-item"
                  onClick={() => { setIsMessageActive(true); setIsNotificationActive(false); setIsSettingsActive(false) }}>
                  <span className="header-list-name">
                ...
                <li className="header-nav-item"
                  onClick={() => {
                    setIsSettingsActive(!isSettingsActive); setIsMessageActive(false); setIsNotificationActive(false)
                  }}>
                  <span className="header-list-name profile-image">
                    <Avatar name={profile?.username} bgColor={profile?.avatarColor} textColor="#fffff" size={40} avatarSrc={profile?.profilePicture} />
                  </span>
