...
const NotificationSettings = () => {
  ...
  const sendNotificationSettings = async() => {
    try {
      ...
      profile = cloneDeep(profile); // we are gonna update notifications object is redux profile
      profile.notifications = response.data.settings; // or notificationSettings
    ...
    <>
      <div className="notification-settings">
        {notificationTypes.map((data, index) => (
          <div className="notification-settings-container" key={Utils.generateString(10)} data-testid="notification-settings-item">
